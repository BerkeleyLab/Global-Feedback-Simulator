  ...
  )
{
  ...
  // Beam impedance (conversion factor between beam current and beam induced voltage)
  // Includes shift to take beam phase relative to the RF into account
  elecMode -> k_beam = RoverQ*Q_L*cexp(-I*rf_phase);
...
double complex ElecMode_Step(
  ...
  double beam_current,             ///< Beam current in Amps
  ...
  )
{
  ...
  // Beam-induced voltage (convert current to voltage and add timing noise)
  v_beam = beam_current * elecMode -> k_beam * cexp(-I*elecMode->LO_w0*delta_tz);  // k_beam = (R/Q) * Q_L
...
double complex Cavity_Step(
  ...
  double complex beam_current, ///< Beam current in Amps (complex since it includes RF phase information)
  ...
  )
{
  ...
  for(i=0;i<cav->n_modes;i++){
    ...
    v_out += ElecMode_Step(cav->elecMode_net[i], Kg_fwd, beam_current, delta_tz, cav_state->elecMode_state_net[i], &v_probe_now, &v_em_now);
