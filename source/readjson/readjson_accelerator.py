...
class ElecMode:
    ...
    def Compute_ElecMode(self, Tstep, rf_phase):
        ...
        bw = w0/(2.0*Q_L);
        k_beam = RoverQ*Q_L*np.exp(-1j*beam_phase);
...
class Controller:
    ...
    def __str__(self):
        ...
        + "control_zero: " + str(self.control_zero) + "\n")
...
class Amplifier:
    ...
    def Get_Saturation_Limit(self):
        ...
        import accelerator as acc
        # Input vector
        inp = np.arange(0.0,20.0,1e-4,dtype=np.complex)
        # Output vector
...
class Station:
    ...
    def Get_C_Pointer(self):
        ...
        NSF_bw = self.ns_filter_bw['value'] # Bandwidth in Hz

        Clip = self.amplifier.Clip['value']
        PAmax = self.amplifier.PAmax['value']
        PAscale = np.sqrt(PAmax) # [sqrt(W)]
        PAbw = self.amplifier.PAbw['value']

        # Limit FPGA output signal not to exceed a SSA output limit.
        # SSA limit is defined as top_drive per-cent of the maximum output power (PAmax)
        FPGA_out_sat = self.amplifier.Get_Saturation_Limit()*PAscale

        stable_gbw = self.controller.stable_gbw['value']
        control_zero = self.controller.control_zero['value']
        ...
        acc.RF_Station_Allocate_In(rf_station, Tstep_global, Clip, PAmax, PAscale, PAbw, NSF_bw, cavity_pointer, stable_gbw, control_zero, FPGA_out_sat, loop_delay_size, probe_ns_rms, rev_ns_rms, fwd_ns_rms)
