"""
Functions for building a linearization matrix of the function doublecompress.c
"""
...
def full_dc_matrix(gun, linp_arr, dv, dphi, Nlinac):
    ...
    It builds a jacobian/sensitivty matrix from all accelerator
    inputs (Voltage and phase) to all accelerator ouputs (dE_E,sz,dt,sd).
    This forms a (4*Nlinac)x(2*Nlinac) matrix. The derivatives to calculate
    the Jacobian are calculated numerically using a midpoint approximation
    and evaluations of doublecompress_new.c

    To make sure everyone is speaking the same language, synonyms for this matrix include:
        Jacobian, Tangent, Derivative, Sensitivity, Small Signal, Gradient, etc...
        dc(x) =~ dc(x0) + [M]*(x-x0) + O((x-x0)^2) ...
        Jacobian matrix is the term used in this file.
    Inputs:
        - gun: linac.gun data structure storing parameters of the electron gun.
            It supplies charge(gun.Q) for use in doublecomprress.
            Should be read in using configuation files
        - linp_arr:  Array of points to linac objects which store parameters for
            the linac. This object is passed to doublecompress and should come
            from reading in configuration files.
        ...
        - Nlinac: Is an integer which is the number of linacs in the accelator
    ...
    Output: The output matrix is the sensitivity matrix
       ...
       Units are SI except energy is in eV. doublecompress.c has a more detailed description.
    """
    ...
    #
    # Set up empty structures to call double compress
    #
    ...
    for j in xrange(Nlinac):
        #
        # Perform the calculations needed for midpoint rule
        #
        ...
        dphivr[j] = 0.0  # set back to zero

        # Calculate derivatives and place into the columns of matrix M
