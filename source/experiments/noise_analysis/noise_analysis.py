...
class Signal:
    ...
    def plot(self, title, xlim=None, ylim=None, plot_type='amplitude', show_limits=False, show_ssa_limits=False, beam_on=False, annotate=False, beam_start=20, beam_end=22, hide_fwd=False):
        ...
        if plot_type == 'amplitude':
            if hide_fwd == False:
                plt.plot(self.trang*1e3,np.abs(self.E_fwd)*fund_k_drive, '-',  label=r'Forward $\left(\vec E_{\rm fwd}\right)$', linewidth=2)
                plt.plot(self.trang*1e3,np.abs(self.E_reverse/fund_k_em), '-', label=r'Reverse $\left(\vec E_{\rm reverse}\right)$', linewidth=2)
            plt.plot(self.trang*1e3,np.abs(self.cav_v), '-', label=r'Cavity Field', linewidth=2, color='r')
            plt.plot(self.trang*1e3,np.abs(self.set_point/fund_k_probe), '-', label=r'Set-point $\left(\vec E_{\rm sp}\right)$', linewidth=2, color='c')
            # Y label
            ...
            if show_ssa_limits == True:
                ...
                plt.plot(self.trang*1e3,np.abs(self.set_point/fund_k_probe)+ssa_limit/np.sqrt(2)/2, label='SSA Upper (RMS) limit', linewidth=2,color='m')
                plt.plot(self.trang*1e3,np.abs(self.set_point/fund_k_probe)-ssa_limit/np.sqrt(2)/2, label='SSA Lower (RMS) limit', linewidth=2, color='y')
        ...
        if plot_type == 'phase':
            ...
            plt.plot(self.trang*1e3,np.angle(self.set_point/fund_k_probe, deg=True), label=r'Set-point $\left(\vec E_{\rm sp}\right)$', linewidth=2, color='c')
            if show_limits:
        ...
        if plot_type == 'cartesian':
            plt.plot(self.trang*1e3,np.real(self.E_fwd)*fund_k_drive, '-',  label=r'Forward $\Re \left(\vec E_{\rm fwd}\right)$', linewidth=2)
            plt.plot(self.trang*1e3,np.imag(self.E_fwd)*fund_k_drive, '-',  label=r'Forward $\Im \left(\vec E_{\rm fwd}\right)$', linewidth=2)
        ...
        if annotate:
            delta_E_fwd_text = r'$\Delta \left| \vec E_{\rm fwd} \right| \approx \,$' + '%.d MV'% (round(np.abs(fund_k_beam)*100e-6*1e-6))
            plt.annotate(delta_E_fwd_text, xy=(21.05, 18e6), fontsize=25)
            index_of_21 = np.where(self.trang==21e-3)
            plt.annotate(s='', xy=(21,np.abs(self.E_fwd[index_of_21])*fund_k_drive), xytext=(21,np.abs(self.cav_v[index_of_21])), arrowprops=dict(arrowstyle='<->') )
...
def run_noise_numerical(Tmax, test_file, beam_configuration, detuning=False):
    ...
    if feedforward:
        feed_forward[(beam_start-0.5e-6)/Tstep:(beam_start+(n_pulses-0.5)*1e-6)/Tstep] = nominal_feed_forward

    # Remove a pulse if option is enabled
    if missing_pulse:
        beam_current[(beam_start+1e-6)/Tstep] = 0.0
        feed_forward[(beam_start+0.5e-6)/Tstep:(beam_start+1.5*1e-6)/Tstep:] = 0.0
