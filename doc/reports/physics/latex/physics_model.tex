...
where the minus sign of the reflection on the cavity coupler is represented in the summing junction. This transformation takes into account the propagation of the reflection of the incident wave back to the directional coupler.
...
as it can be deduced from Fig.~\ref{fig:RF_cavity_block_diagram}. 
...
Equate coefficients with the earlier low-pass filter form, in the case $Q > \frac{1}{2}$, to get
...
which is equivalent to scaling $\vec V_{\rm out}^n$ by a factor of $|p|$ in the software implementation.
