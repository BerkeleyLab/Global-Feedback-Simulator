...
def iodicttocolumn_fullNumpyarray(data,iodict, connect=None):
    #routine that can be used when loadall data is used
    #I do not recommend using this function or load all
    #please use iodict_to_column_num to get column number
    # and then use numpy.loadtxt with the usecols keyword
    # to load coloumn out of the text file directly
    print connect
    if (connect):
        linidx = { connect[i]:i for i in range(len(connect)) }
...
def loadalldata(datafile):
    #this routine will read in all the data in a output file
    #if output data is large this is not advisable
    #input: string to data txt file
    #ouput: numpy array
